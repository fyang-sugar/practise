 ...
 * @return {boolean}
 ...
 To check whether words1[i] and words2[i] are similar, either they are the same word, 
...
var areSentencesSimilar = function(words1, words2, pairs) {
    ...
    for(var i =0; i<words1.length; i++) {
        if(words1[i] !== words2[i] && !set.has(words1[i] +':'+words2[i]) && !set.has(words2[i] +':'+words1[i])) {
            return false;
        }
    }
    return true;
